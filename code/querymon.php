...
function imfsMonitorGather() {
	...
	$queryLogSizeThreshold = 1024 * 1024;
	...
	foreach ( $queries as $q ) {
		try {
			...
			if ( $method != 'SHOW' ) {
				$logEntry        = (object) [];
				$fingerprint     = $parser->getFingerprint();
				$qid             = substr( hash( 'md5', $fingerprint, false ), 0, 16 );
				$logEntry->f     = $fingerprint;
				$logEntry->t     = $q[1];
				$logEntry->n     = 1;
				$logEntry->admin = ! ! is_admin();
				if ( array_key_exists( $qid, $queryLog->queries ) ) {
					$queryLog->queries[ $qid ]->n     += 1;
					$queryLog->queries[ $qid ]->t     += $logEntry->t;
					$queryLog->queries[ $qid ]->admin = $logEntry->admin;
