...
class ImfsDb {
	...
	/**
	 * @return Generator list of tables manipulated by this module
	 */
	...
	public function checkTable( $action, $name ): bool {
		...
		$result  = true;
		$stmt    = $wpdb->prepare( $this->queries['indexes'], $table );
		$indexes = $wpdb->get_results( $stmt, OBJECT_K );
	...
	protected function rekeyTable( $action, $name ) {
		...
		$table = $wpdb->prefix . $name;
		if ( $action ) {
			$operations = $stmts[ $action ];
			foreach ( $operations as $fragment ) {
	 ...
	 * @param $action "enable" or "disable"
	 *
	 * @return array of strings with messages describing problems, or falsey if no problems
	 */
	public function anyProblems( $action ): array {
		$problems = false;
		$this->clearMessages();
	...
	public function rekey( $action ) {
		$problems = $this->$this->anyProblems( $action );
		if ( ! $problems ) {
			foreach ( $this->tables() as $name ) {
				if ( $this->checkTable( $action, $name ) ) {
