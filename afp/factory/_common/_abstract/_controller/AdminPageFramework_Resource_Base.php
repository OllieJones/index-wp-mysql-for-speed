...
abstract class Imfs_AdminPageFramework_Resource_Base extends Imfs_AdminPageFramework_FrameworkUtility {
    protected static $_aStructure_EnqueuingResources = array('sSRC' => null, 'sSRCRaw' => null, 'aPostTypes' => array(), 'sPageSlug' => null, 'sTabSlug' => null, 'sType' => null, 'handle_id' => null, 'dependencies' => array(), 'version' => false, 'translation' => array(), 'in_footer' => false, 'media' => 'all', 'attributes' => array(),);
    protected $_sClassSelector_Style = 'index-wp-mysql-for-speed-style';
    protected $_sClassSelector_Script = 'index-wp-mysql-for-speed-script';
    protected $_aHandleIDs = array();
    public $oProp;
    public $oUtil;
    public function __construct($oProp) {
        $this->oProp = $oProp;
        $this->oUtil = new Imfs_AdminPageFramework_WPUtility;
        if ($this->isDoingAjax()) {
            return;
        }
    ...
    private function ___getCommonStyleTag($oCaller, $sIDPrefix) {
        $_sStyle = $this->addAndApplyFilters($oCaller, array("style_common_admin_page_framework", "style_common_{$this->oProp->sClassName}",), Imfs_AdminPageFramework_CSS::getDefaultCSS());
    ...
    private function ___getCommonIEStyleTag($oCaller, $sIDPrefix) {
        $_sStyleIE = $this->addAndApplyFilters($oCaller, array("style_ie_common_admin_page_framework", "style_ie_common_{$this->oProp->sClassName}",), Imfs_AdminPageFramework_CSS::getDefaultCSSIE());
    ...
    protected function _printCommonScripts($sIDPrefix, $sClassName) {
        ...
        $_sScript = $this->addAndApplyFilters($this->oProp->oCaller, array("script_common_admin_page_framework", "script_common_{$this->oProp->sClassName}",), Imfs_AdminPageFramework_Property_Base::$_sDefaultScript);
    ...
    public function _addEnqueuingResourceByType($sSRC, array $aCustomArgs = array(), $sType = 'style') {
        ...
        $this->oProp->{$_sContainerPropertyName}[$_sSRC] = array_filter($this->getAsArray($aCustomArgs), array($this, 'isNotNull')) + array('sSRCRaw' => $_sRawSRC, 'sSRC' => $_sSRC, 'sType' => $sType, 'handle_id' => $sType . '_' . $this->oProp->sClassName . '_' . (++$this->oProp->{$_sEnqueuedIndexPropertyName}),) + self::$_aStructure_EnqueuingResources;
