...
class Imfs_AdminPageFramework_Form_View___Script_RepeatableField extends Imfs_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updateImfs_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.index-wp-mysql-for-speed-repeatable-field-buttons' )
            .filter( function() {                                       
                return $( this ).closest( '.index-wp-mysql-for-speed-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            });       
        ...
        _oRepeatableAddButtons.on( 'click', function() {
            ...
            $( this ).addImfs_AdminPageFrameworkRepeatableField();
        ...
        _oRepeatableRemoveButton.on( 'click', function() {
            ...
            $( this ).removeImfs_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.index-wp-mysql-for-speed-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.index-wp-mysql-for-speed-field' )
            .filter( function() {                 
                return $( this ).closest( '.index-wp-mysql-for-speed-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
            })        
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addImfs_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addImfs_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.index-wp-mysql-for-speed-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.index-wp-mysql-for-speed-fields' );
        ...
        if ( ! _aOptions ) {     
            var _nodeButtonContainer = nodeFieldContainer.find( '.index-wp-mysql-for-speed-repeatable-field-buttons' )
                .filter( function() {                 
                    return $( this ).closest( '.index-wp-mysql-for-speed-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.index-wp-mysql-for-speed-field' ) 
                                    .filter( function() {                 
                                        return $( this ).closest( '.index-wp-mysql-for-speed-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.index-wp-mysql-for-speed-repeatable-field-buttons' )
                                    .filter( function() {                 
                                        return $( this ).closest( '.index-wp-mysql-for-speed-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.                
                                    }); 
         ...
         */
        nodeNewField.updateImfs_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.index-wp-mysql-for-speed-field' ).addBack().trigger( 
            'index-wp-mysql-for-speed_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.index-wp-mysql-for-speed-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {                      
                return $( this ).closest( '.index-wp-mysql-for-speed-fields' ).attr( 'id' ) === _sFieldsContainerID;                
            })
        ...
        nodeFieldsContainer.children( '.index-wp-mysql-for-speed-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.index-wp-mysql-for-speed-field,.index-wp-mysql-for-speed-fields,.index-wp-mysql-for-speed-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removeImfs_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.index-wp-mysql-for-speed-field' );
        var nodeFieldsContainer = $( this ).closest( '.index-wp-mysql-for-speed-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
