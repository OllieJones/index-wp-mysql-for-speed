...
class Imfs_AdminPageFramework_FieldType_system extends Imfs_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".index-wp-mysql-for-speed-field-system {width: 100%;}.index-wp-mysql-for-speed-field-system .index-wp-mysql-for-speed-input-label-container {width: 100%;}.index-wp-mysql-for-speed-field-system textarea {background-color: #f9f9f9; width: 97%; outline: 0; font-family: Consolas, Monaco, monospace;white-space: pre;word-wrap: normal;overflow-x: scroll;}";
    }
    protected function getField($aField) {
        $_aInputAttributes = $aField['attributes'];
        $_aInputAttributes['class'].= ' system';
        unset($_aInputAttributes['value']);
        return $aField['before_label'] . "<div class='index-wp-mysql-for-speed-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'index-wp-mysql-for-speed-input-label-string') . ">" . $aField['label'] . "</span>" : "") . "<textarea " . $this->getAttributes($_aInputAttributes) . " >" . esc_textarea($this->___getSystemInformation($aField['value'], $aField['data'], $aField['print_type'])) . "</textarea>" . $aField['after_input'] . "</label>" . "</div>" . $aField['after_label'];
    }
    ...
    private function ___getFormattedSystemInformation($asCustomData) {
        ...
        $_aData = $_aData + array('WordPress' => $this->___getSiteInfoWithCache(!isset($_aData['WordPress'])), 'PHP' => $this->_getPHPInfo(!isset($_aData['PHP'])), 'PHP Error Log' => $this->___getErrorLogByType('php', !isset($_aData['PHP Error Log'])), 'MySQL' => isset($_aData['MySQL']) ? null : $this->getMySQLInfo(), 'MySQL Error Log' => $this->___getErrorLogByType('mysql', !isset($_aData['MySQL Error Log'])), 'Server' => $this->___getWebServerInfo(!isset($_aData['Server'])), 'Browser' => $this->___getClientInfo(!isset($_aData['Browser'])), 'Admin Page Framework' => isset($_aData['Admin Page Framework']) ? null : Imfs_AdminPageFramework_Registry::getInfo(),);
    ...
    private function ___getSiteInfo() {
        global $wpdb;
        return array(__('Version', 'index-wp-mysql-for-speed') => $GLOBALS['wp_version'], __('Language', 'index-wp-mysql-for-speed') => $this->getSiteLanguage(), __('Memory Limit', 'index-wp-mysql-for-speed') => $this->getReadableBytes($this->getNumberOfReadableSize(WP_MEMORY_LIMIT)), __('Multi-site', 'index-wp-mysql-for-speed') => $this->getAOrB(is_multisite(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Permalink Structure', 'index-wp-mysql-for-speed') => get_option('permalink_structure'), __('Active Theme', 'index-wp-mysql-for-speed') => $this->___getActiveThemeName(), __('Registered Post Statuses', 'index-wp-mysql-for-speed') => implode(', ', get_post_stati()), 'WP_DEBUG' => $this->getAOrB($this->isDebugMode(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_LOG' => $this->getAOrB($this->isDebugLogEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), 'WP_DEBUG_DISPLAY' => $this->getAOrB($this->isDebugDisplayEnabled(), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Table Prefix', 'index-wp-mysql-for-speed') => $wpdb->prefix, __('Table Prefix Length', 'index-wp-mysql-for-speed') => strlen($wpdb->prefix), __('Table Prefix Status', 'index-wp-mysql-for-speed') => $this->getAOrB(strlen($wpdb->prefix) > 16, $this->oMsg->get('too_long'), $this->oMsg->get('acceptable')), 'wp_remote_post()' => $this->___getWPRemotePostStatus(), 'wp_remote_get()' => $this->___getWPRemoteGetStatus(), __('Multibite String Extension', 'index-wp-mysql-for-speed') => $this->getAOrB(function_exists('mb_detect_encoding'), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('WP_CONTENT_DIR Writeable', 'index-wp-mysql-for-speed') => $this->getAOrB(is_writable(WP_CONTENT_DIR), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Active Plugins', 'index-wp-mysql-for-speed') => PHP_EOL . $this->___getActivePlugins(), __('Network Active Plugins', 'index-wp-mysql-for-speed') => PHP_EOL . $this->___getNetworkActivePlugins(), __('Constants', 'index-wp-mysql-for-speed') => $this->___getDefinedConstants('user'),);
    }
    ...
    private function ___getWebServerInfo($bGenerateInfo = true) {
        return $bGenerateInfo ? array(__('Web Server', 'index-wp-mysql-for-speed') => $_SERVER['SERVER_SOFTWARE'], 'SSL' => $this->getAOrB(is_ssl(), $this->oMsg->get('yes'), $this->oMsg->get('no')), __('Session', 'index-wp-mysql-for-speed') => $this->getAOrB(isset($_SESSION), $this->oMsg->get('enabled'), $this->oMsg->get('disabled')), __('Session Name', 'index-wp-mysql-for-speed') => esc_html(@ini_get('session.name')), __('Session Cookie Path', 'index-wp-mysql-for-speed') => esc_html(@ini_get('session.cookie_path')), __('Session Save Path', 'index-wp-mysql-for-speed') => esc_html(@ini_get('session.save_path')), __('Session Use Cookies', 'index-wp-mysql-for-speed') => $this->getAOrB(@ini_get('session.use_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')), __('Session Use Only Cookies', 'index-wp-mysql-for-speed') => $this->getAOrB(@ini_get('session.use_only_cookies'), $this->oMsg->get('on'), $this->oMsg->get('off')),) + $_SERVER : '';
    }
    }
