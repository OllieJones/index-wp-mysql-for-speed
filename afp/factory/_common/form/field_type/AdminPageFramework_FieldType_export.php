...
class Imfs_AdminPageFramework_FieldType_submit extends Imfs_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".index-wp-mysql-for-speed-field input[type='submit'] {margin-bottom: 0.5em;}.index-wp-mysql-for-speed-field-submit .submit-confirm-container {display: inline-block;margin-left: 1em;vertical-align: middle;}.index-wp-mysql-for-speed-field-submit .field-error.submit-confirmation-warning {float: none;}";
    }
    protected function getScripts() {
        return <<<JAVASCRIPTS
jQuery( document ).ready( function(){
    jQuery( '.index-wp-mysql-for-speed-field-submit .submit-confirm-container input[type=checkbox]' ).each( function( index, value ){
        jQuery( this ).closest( '.index-wp-mysql-for-speed-field-submit' ).find( 'input[type=submit]' ).on( 'click', function( event ){
            var _fieldSubmit = jQuery( this ).closest( '.index-wp-mysql-for-speed-field-submit' );  
            _fieldSubmit.find( '.submit-confirmation-warning' ).remove(); // previous error message
            var _confirmCheckbox = jQuery( this ).closest( '.index-wp-mysql-for-speed-field-submit' ).find( '.submit-confirm-container input[type=checkbox]' );
...
JAVASCRIPTS;
    ...
    private function _getLabelContainerAttributes(array $aField) {
        return array('style' => $aField['label_min_width'] || '0' === ( string )$aField['label_min_width'] ? "min-width:" . $this->getLengthSanitized($aField['label_min_width']) . ";" : null, 'class' => 'index-wp-mysql-for-speed-input-label-container' . ' index-wp-mysql-for-speed-input-button-container' . ' index-wp-mysql-for-speed-input-container',);
    }
    ...
    private function _getHiddenInput_Email(array $aField) {
        ...
        $this->setTransient('apf_em_' . md5($aField['_input_name_flat'] . get_current_user_id()), array('nonce' => $this->getNonceCreated('apf_email_nonce_' . md5(( string )site_url()), 86400),) + $this->getAsArray($aField['email']));
    ...
    class Imfs_AdminPageFramework_FieldType_export extends Imfs_AdminPageFramework_FieldType_submit {
